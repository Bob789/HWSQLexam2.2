...
CREATE TABLE tourists (
    tourists_id SERIAL PRIMARY KEY,
...
CREATE TABLE tours_tourists_connection (
    tours_id INT NOT NULL,
    tourists_id INT NOT NULL,
    PRIMARY KEY (tours_id, tourists_id),
    FOREIGN KEY (tours_id) REFERENCES tours(tours_id) ON DELETE CASCADE,
    FOREIGN KEY (tourists_id) REFERENCES tourists(tourists_id) ON DELETE CASCADE
);

INSERT INTO tours_tourists_connection (tours_id,tourists_id) VALUES
(5,9),
(2,8),
(10,25),
(1,4),
...
SELECT
	...
	t2.description,
	t2.start_date,
	t2.end_date, t2.price,
	t2.max_participants,
	t2.guide_name,
	t2.difficulty_level,
	t2.pickup_location
FROM tourists t
join tours_tourists_connection ttc on ttc.tourists_id = t.tourists_id
join tours t2 on t2.tours_id = ttc.tours_id ;
